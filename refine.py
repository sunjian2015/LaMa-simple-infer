...
from modules import FFCResnetBlock
from utils import pad_tensor_to_modulo, move_to_device
...
def _pyrdown_mask(
        mask : torch.Tensor, downsize : tuple=None, eps : float=1e-8,
        blur_mask : bool=True, round_up : bool=True
    ):
...
def _infer(
        ...
        forward_front : nn.Module, forward_rear : nn.Module, 
        ref_lower_res : torch.Tensor, orig_shape : tuple, device : str, 
...
def refine_predict(
        batch : dict, inpainter : nn.Module, device : str, 
    ...
    for idl in range(len(inpainter.model)):
        if isinstance(inpainter.model[idl], FFCResnetBlock): # or isinstance(inpainter.model[idl], ResnetBlock):
            found_first_resblock = True
        elif not found_first_resblock:
            first_resblock_ind += 1

    forward_front = inpainter.model[:first_resblock_ind]
    forward_rears = inpainter.model[first_resblock_ind:]
    forward_front.to(device)
    forward_rears.to(device)
    ...
    for ids, (image, mask) in enumerate(zip(ls_images, ls_masks)):
        ...
        mask[mask >= 1e-8] = 1.0
        mask[mask < 1e-8] = 0.0
        image, mask = move_to_device(image, device), move_to_device(mask, device)
        if image_inpainted is not None:
            image_inpainted = move_to_device(image_inpainted, device)
        image_inpainted = _infer(image, mask, forward_front, forward_rears, image_inpainted, orig_shape, device, ids, n_iters, lr)
